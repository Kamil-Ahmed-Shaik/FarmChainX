...
export default function DistributorProfile() {
  ...
  useEffect(() => {
    axios.get(`/distributor/${userId}/profile`)
  ...
  if (loading) return (<DashboardLayout role="distributor"><p>Loading...</p></DashboardLayout>);
  if (status === "true") return (<DashboardLayout role="distributor" display={false}><h2>Access Restricted</h2><p>You are blocked by admin</p></DashboardLayout>);

  return (
    <DashboardLayout role="distributor" display={true}>
      <header className="dashboard-header">
        <h2 className="dashboard-title">Distributor Profile</h2>
      </header>
      ...
      <div className="form-container">
        <div className="form-row">
          ...
          <div>
            ...
            <div className="form-group"><label>Company Name</label><input value={data.companyName || ""} onChange={(e) => handleChange("companyName", e.target.value)} /></div>
            <div className="form-group"><label>Region</label><input value={data.region || ""} onChange={(e) => handleChange("region", e.target.value)} /></div>
          </div>
        </div>
